// EXAMPLE TESTS: SHOWS FUNCTION THAT CAN BE USED TO SEND SIGNATURE TO SMART CONTRACT FROM SERVER
...
const stripAndPadHexValue = (hexVal, sizeInBytes, start = true) => {
  ...
  return start
    ? strippedHexVal.padStart(sizeInBytes * 2, 0)
...
contract(
  ...
  ([_, owner, authorizedUser, anyone, bouncerAddress, newBouncer]) => {
    before(async function() {
      ...
      this.uintValue = 23;
      this.checkValidSignatureAndMethodId = getMethodId(
      ...
      this.validateSignatureAndDataMsgData = [
        ...
        stripAndPadHexValue(
          web3.toHex(this.bytesValue.substr(2).length / 2),
          32
        ), // bytesValue size
    ...
    it("should allow owner to add a bouncer", async function() {
      ...
      const hasRole = await this.bouncer.hasRole(
        bouncerAddress,
        this.roleBouncer
      );
    ...
    context("modifiers", () => {
      it("should allow valid signature for sender", async function() {
        await this.bouncer.onlyWithValidSignature(this.genSig(authorizedUser), {
          from: authorizedUser
        });
      });
      it("should not allow invalid signature for sender", async function() {
        await assertRevert(
          this.bouncer.onlyWithValidSignature("abcd", {
            from: authorizedUser
          })
        );
      });
      it("should allow valid signature with a valid method for sender", async function() {
        ...
        await this.bouncer.onlyWithValidSignatureAndMethod(sig, {
          from: authorizedUser
        });
      });
      it("should not allow invalid signature with method for sender", async function() {
        await assertRevert(
          this.bouncer.onlyWithValidSignatureAndMethod("abcd", {
            from: authorizedUser
          })
        );
      });
      it("should allow valid signature with a valid data for sender", async function() {
        ...
        await this.bouncer.onlyWithValidSignatureAndData(this.uintValue, sig, {
          from: authorizedUser
        });
      });
      it("should not allow invalid signature with data for sender", async function() {
        await assertRevert(
          this.bouncer.onlyWithValidSignatureAndData(this.uintValue, "abcd", {
            from: authorizedUser
          })
        );
      });
    ...
    context("signatures", () => {
      ...
      it("should not accept invalid message for invalid user", async function() {
        const isValid = await this.bouncer.checkValidSignature(anyone, "abcd");
        isValid.should.eq(false);
      });
      ...
      it("should not accept an invalid method with valid params for valid user", async function() {
        this.validateSignatureAndDataMsgData[0] = getMethodId(
          "invalidMethod",
          "address",
          "bytes",
          "uint256",
    ...
    context("management", () => {
      ...
      it("should be able to add bouncers", async function() {
        await this.bouncer.addBouncer(newBouncer, { from: owner }).should.be
          .fulfilled;
      });

      it("should not allow adding invalid address", async function() {
        await assertRevert(this.bouncer.addBouncer("0x0", { from: owner }));
      });
      ...
      it("should be able to remove bouncers", async function() {
        await this.bouncer.removeBouncer(newBouncer, { from: owner }).should.be
          .fulfilled;
      });
    });
  }
