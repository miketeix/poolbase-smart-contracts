/* **** Truffle exec script run using the command:
    truffle exec ./scripts/createPool.js
**** */
...
const eventEmitterAddress = fs.readFileSync(
    '../../addresses/poolbaseEventEmitter',
    'utf8'
);
const factoryAddress = fs.readFileSync(
    '../../addresses/poolbaseFactory',
    'utf8'
);
...
module.exports = async function(callback) {
    ...
    const {
        events: {
            ContractInstantiation: {
                returnValues: { instantiation: poolAddress }
            }
        }
    ...
    const instantiationCount = await poolFactory.methods
        .getInstantiationCount(coinbaseAccount)
        .call();
